...
class ImageSimilarityLibrary:
    ...
    def _compare_images(self, expected_image_path, actual_image_url):
        expected_image = read_image(expected_image_path)
        actual_image = read_image_from_url(actual_image_url)
        actual_image = resize_image(actual_image, image_size(expected_image))
        # write_image_to_file(actual_image, "actual_image.jpg")  # just debug

        diff1 = compare_images_opencv(actual_image, expected_image)
        diff2 = compare_images_pillow(actual_image, expected_image)
        diff3 = compare_images_scikit(actual_image, expected_image)
        diff4 = compare_images_imagehash(actual_image, expected_image)
...
def write_image_to_file(cv2_image, file_path):
...
def compare_images_pillow(image1, image2):
...
def compare_images_scikit(image1, image2):
    gray1 = cv2.cvtColor(image1, cv2.COLOR_BGR2GRAY)
    gray2 = cv2.cvtColor(image2, cv2.COLOR_BGR2GRAY)
...
def compare_images_imagehash_OLD(image1, image2):
...
def compare_images_imagehash(image1, image2):
